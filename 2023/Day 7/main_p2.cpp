...
int hand_type(string hand)
{
    ...
    for (auto p: cards) if (p.fi != '0') amts.push_back(p.sc);
    sort(amts.begin(), amts.end(), greater<int>());

    if (amts.size() == 0) amts.push_back(0);
    amts[0] += cards['0'];
    for (auto i: amts){
        if (i >= 4) type = i + 3; // Five of a Kind and Four of a Kind
        else if (i == 3){
